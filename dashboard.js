...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function fetchStats() {
    try {
      ...
      // データがない場合の初期化
      if (!response.data.remoteIp) response.data.remoteIp = [];
      if (!response.data.server) response.data.server = [];
      if (!response.data.port) response.data.port = [];
      ...
      // エラー時に空の配列を設定して、後続の処理でエラーが発生しないようにする
      state.data.stats.remoteIp = [];
      state.data.stats.server = [];
      state.data.stats.port = [];
      throw error;
    }
  }
  ...
  function updateStats() {
    try {
      ...
      // ユニークIP数 - state.data.stats.remoteIp が存在するか確認
      const uniqueIps = new Set(
        (state.data.stats.remoteIp || []).map(item => item.value)
      );
      elements.uniqueIpCount.textContent = uniqueIps.size || 0;
      
      // 最多攻撃元IP - state.data.stats.remoteIp が存在し、かつ空でないことを確認
      ...
      elements.uniqueIpCount.textContent = '0';
      elements.topAttacker.textContent = '-';
    }
  }
